...
def createMYSQLDB(db, rwuser, rwpwd, rouser, ropwd, rwhost, rootpwd, drop, schemapath):
    ...
    res = execute(
        cmd + ' -e "SELECT 1 FROM mysql.user ' "WHERE user = '{}'\"".format(rwuser)
    )
    if res.error:
        print("  + {}".format(res.error))
        return False
    ...
    if rwuser != rouser:
        res = execute(
            cmd + ' -e "SELECT 1 FROM mysql.user ' "WHERE user = '{}'\"".format(rouser)
        )
        if res.error:
            print("  + {}".format(res.error))
            return False
        ...
        exists = "1" in res.data
        if not exists:
            q += "CREATE USER '{}'@'localhost' IDENTIFIED BY '{}';".format(
                rouser, ropwd
            )
...
def main():
    if len(sys.argv) != 10:
        print(
            "Usage: mysql_setup.py <db> <rwuser> <rwpwd> <rouser> <ropwd> "
    ...
    if not createMYSQLDB(
        db, rwuser, rwpwd, rouser, ropwd, rwhost, rootpwd, drop, schemapath
    ):
        return 1
